...
extension LXFWeChatTools {
    ...
    func autoLogin() -> (Bool) {
        ...
        var request = NIMUserRequest.init()
        request.userId = "15755168422";
        request.operation = NIMUserOperation(rawValue: 1)!
        request.message = "跪求通过"

        NIMSDK.shared().userManager.requestFriend(request) { (_) in
...
extension LXFWeChatTools {
    ...
    func refreshFriends() {
        ...
        addFriend("xunan623", message: "混合") { (_) in
